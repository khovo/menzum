...
async def handle_callback(session, db, cb: dict, channels: list[dict]) -> None:
    ...
    if data_str.startswith("lyrics_approve_") and _is_admin(user_id):
        ...
        if not lyrics_doc:
            await answer_callback_query(session, cb_id, "⚠️ Doc not found or already processed.", show_alert=True)
            return
        # Flip has_lyrics flag on the files doc → O(1) reads in featured/search/library
        await set_file_has_lyrics(db, lyrics_doc["file_id"], True)
        track_name   = lyrics_doc.get("track_name", "Unknown")
        submitter_id = lyrics_doc.get("submitted_by")
        if submitter_id:
            await send_message(
                session, submitter_id,
                f"✅ እናመሰግናለን! ለ {track_name} የላኩት ግጥም ፀድቋል።",
            )
        await edit_message_text(
            session, chat_id, message_id,
            f"✅ Approved lyrics for: `{track_name}`",
        )
        await answer_callback_query(session, cb_id)
        return

    if data_str.startswith("lyrics_reject_") and _is_admin(user_id):
        doc_id     = data_str[len("lyrics_reject_"):]
        lyrics_doc = await reject_lyrics(db, doc_id)
        if not lyrics_doc:
            await answer_callback_query(session, cb_id, "⚠️ Doc not found or already processed.", show_alert=True)
            return
        # has_lyrics stays False/absent — no files update needed on rejection
        track_name   = lyrics_doc.get("track_name", "Unknown")
        submitter_id = lyrics_doc.get("submitted_by")
        if submitter_id:
            await send_message(
                session, submitter_id,
                f"❌ ይቅርታ፣ ለ {track_name} የላኩት ግጥም አልፀደቀም። እባክዎ አስተካክለው እንደገና ይሞክሩ።",
            )
        await edit_message_text(
            session, chat_id, message_id,
            f"❌ Rejected lyrics for: `{track_name}`",
        )
        await answer_callback_query(session, cb_id)
        return
