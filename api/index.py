...
# 🔥 SPEED OPTIMIZATION: Cached Membership Check
                ...
                try:
                    if len(doc_id) == 24:
                        ...
                        if file_exists:
                            ...
                            await answer_callback_query(cb_id, "⚠️ Missing")
                    else:
            ...
            if data_str.startswith("report_") or data_str.startswith("broadcast_"):
                ...
                if data_str.startswith("report_"):
                    ...
                    try:
                        ...
                        if file_doc:
                            await send_message(ADMIN_ID, f"🚨 Report: `{file_doc.get('display_name')}`\nID: `{doc_id}`")
                            await answer_callback_query(cb_id, "✅ Reported!", show_alert=True)
                    except: pass
                    ...
                    if msg_id:
                        ...
                        async for u in users:
                            try:
                                await copy_message(u["_id"], chat_id, msg_id, reply_markup=markup)
                                count += 1
                                await asyncio.sleep(0.05) 
                            except: pass
                        await send_message(chat_id, f"✅ Sent to {count}")
                        await set_user_state(db, user_id, "idle")
                    await answer_callback_query(cb_id)
