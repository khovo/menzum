        ...
        if "inline_query" in data:
            ...
            if db is None:
                results.append({
                    ...
                    "description": "Please check Vercel logs.",
                    "input_message_content": {"message_text": "DB Connection Failed"}
                })
            else:
                cursor = None
                # Search Logic
                ...
                if cursor:
                    ...
                    if count == 0 and skip == 0:
                         results.append({
                            ...
                            "id": "no_res",
                            "title": "📭 ምንም አልተገኘም",
                            "description": f"ለ '{query}' ምንም ውጤት የለም።",
                            "input_message_content": {"message_text": f"No results for {query}"}
                        })
            
            # Next Offset
            next_offset = str(skip + limit) if results and len(results) >= limit else ""
