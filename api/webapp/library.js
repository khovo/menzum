  ...
  try {
    ...
    const favDocs = favoriteFileIds.length > 0
      ? await db.collection("files")
          .find(
            { file_id: { $in: favoriteFileIds }, hidden: { $ne: true }, hidden_app: { $ne: true } },
            { projection: { display_name: 1, thumb_file_id: 1, thumb_url: 1 } }   // never expose file_id
          )
          .limit(50)
          .toArray()
      : [];
    ...
    const pdfFavDocs = pdfFavoriteIds.length > 0
      ? await db.collection("pdfs")
          .find(
            { _id: { $in: pdfFavoriteIds.map((id) => { try { return new ObjectId(id); } catch { return null; } }).filter(Boolean) }, hidden: { $ne: true }, hidden_app: { $ne: true } },
